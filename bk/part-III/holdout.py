...
environment = bucket.loadEnvironment(path='./environment.yaml')
source = bucket.Source(train=environment['train'], test=environment['test'])
source.loadStructure()
# source.splitValidation(stratification='label')


bucket.createSet(source.structure.train)
